...
NOMBRES_M  = ['Carlos','José','Miguel','Luis','Roberto',
               'Juan','Ricardo','Alejandro','Fernando','David']
NOMBRES_F  = ['María','Ana','Patricia','Laura','Carmen',
               'Rosa','Sandra','Verónica','Diana','Claudia']
APELLIDOS  = ['Herrera','González','Martínez','Rodríguez','López',
               'García','Pérez','Sánchez','Ramírez','Torres',
               'Díaz','Flores','Castro','Vargas','Morales',
               'Reyes','Jiménez','Cruz','Mendoza','Ruiz']

def cedula_panama(i):
    provincia = str((i % 9) + 1)
    tomo      = str(100 + (i // 9999) % 900).zfill(3)
    asiento   = str(1000 + (i % 8999)).zfill(4)
...
PROVINCIAS_PA = [
    'Panamá','Panamá Oeste','Chiriquí','Veraguas','Colón',
...
if __name__ == '__main__':
    ...
    conn   = get_connection()
    cursor = conn.cursor()
    cursor.execute("DELETE FROM fact.reposicion")
    cursor.execute("DELETE FROM fact.inventario")
    cursor.execute("DELETE FROM fact.fidelidad")
    cursor.execute("DELETE FROM fact.ventas")
    cursor.execute("DELETE FROM dim.promocion")
    cursor.execute("DELETE FROM dim.cliente")
    cursor.execute("DELETE FROM dim.empleado")
    cursor.execute("DELETE FROM dim.articulo")
    cursor.execute("DELETE FROM dim.categoria")
    conn.commit()
    conn.close()
    print("🗑️  Tablas limpiadas en orden correcto.")

    print("\n[1/4] Generando e insertando categorías...")
    df_cat = generar_categorias()
    insertar_dataframe(df_cat, 'dim.categoria', limpiar=True, truncar=False)

    print("\n[2/4] Generando e insertando artículos...")
    df_art = generar_articulos()
    insertar_dataframe(df_art, 'dim.articulo', limpiar=True, truncar=False)
    print(f"       Artículos con frío : {df_art['requiere_frio'].sum()}")
    print(f"       Margen promedio    : "
          f"{((df_art['precio_base']-df_art['costo'])/df_art['precio_base']*100).mean():.1f}%")

    print("\n[3/4] Generando e insertando empleados...")
    df_emp = generar_empleados()
    insertar_dataframe(df_emp, 'dim.empleado', limpiar=True, truncar=False)
    print(f"       Total empleados: {len(df_emp)}")

    print("\n[4/4] Generando e insertando clientes...")
    df_cli = generar_clientes(500)
    insertar_dataframe(df_cli, 'dim.cliente', limpiar=True, truncar=False)
